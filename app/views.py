...
def inpositions(text):
    positions = ["Representative", "Designer", "Engineer", "Technician", "Developer", "Mentor", "Assistant",
                 "Counselor", "Secretary", "Manager", "Leader"]
    for p in positions:
        if text.find(p) != -1:
            return True
    return False
...
def MakeForm(text):
    ...
    for line in lines:
        ...
        if bool(re.search(r'\s{0,2}skills\s{0,2}$', check_line)) == True or line.find("Skills") != -1 or line.find(
                "SKILLS") != -1:
            skillmode = 1
            flag = 0
            count = 0
        ...
        if 'responsibility' in check_line or 'education' in check_line or dictionary['Name'] in line or dictionary[
            'Phone'] in line or dictionary['Email'] in line:
            continue

        if line == "  ":
            flag = 0
            count = 0
        if (check_line.find('internships') != -1) or bool(
                re.search(r'\s{0,2}experience\s{0,2}$', check_line)) == True or (
                check_line.find('employment history') != -1):
            # print(line)
            flag = 2
            count = 0
            skillmode = 0
            continue
        if (check_line.find('projects') != -1) or bool(
                re.search(r'\s{0,2}project\s{0,2}$', check_line)) == True or bool(
            re.search(r'\s{0,2}projects\s{0,2}$', check_line)) == True:
            flag = 3
            count = 0
            skillmode = 0
            continue
        ...
        if flag == 4:
            if count == 0:
                ...
                if bool(re.search(r'[0-9]{4}', check_line)) == True and line.find(
                        "https://") == -1 and 'Date' not in train.keys():
                    l = re.findall(r'[0-9]{4}', check_line)
                    # print(l)
                    if len(l) == 1:
                        start_date = l[0] + "-01-01"
        ...
        for deg in degrees:
            ...
            for line in lines:
                ...
                if check_line.find(deg) != -1:
                    ...
                    if check_line.find("grade") != -1 or check_line.find("gpa") != -1 or check_line.find(
                            "cgpa") != -1 or check_line.find("%") != -1 and 'Grade' not in edu:
...
def save(request):
    if request.method == "POST":
        ...
        obj = social(email=email, dob=dob, gender=gender, martial=marital, hometown=address, hobbies=hobbies,
                     mobile_number=phone, linkedin_profile=linkedin, facebook_profile=github)
        obj.save()
        ...
        # return HttpResponse("You need to add atleast one education field!")
            ...
            for i in range(1, total + 1):
                ...
                grade = 'eGrade' + str(i)
                field = 'efield' + str(i)
                if request.POST.get(deg) is None:
                    continue
                ...
                obj = academic(name=name, email=email, school_or_college=request.POST.get(inst), degree_id=degree_id,
                               field_of_study_id=field_of_study_id, start_date=request.POST.get(date),
                               end_date="2020-01-01", grade=request.POST.get(grade))
                obj.save()
        ...
        if 'skill1' not in dict.keys():
            # return HttpResponse("You need to add at least one skill field!")
            pass
        else:
            ...
            for i in range(1, total + 1):
                ...
                if skillname in dict.keys():
                    ...
                    # Check if the skill contains commas
                    if ',' in skill:
                        # Split the skill name by commas
                        skills_list = [item.strip() for item in skill.split(",")]
                        for skill_name in skills_list:
                            obj = add_skill(email=email, name=skill_name)
                            obj.save()
                            recommend_applicants_for_job(request, [obj.name])
                    else:
